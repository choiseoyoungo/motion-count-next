  ...
  switch (name) {
    ...
    case 'PermissionDeniedError':
      return {
        ...
        message:
          '브라우저가 이 사이트의 카메라 접근을 차단했습니다. 사이트 설정에서 카메라 권한을 허용한 뒤 다시 시작해주세요.',
      }
    case 'NotFoundError':
    ...
    case 'TrackStartError':
      return {
        ...
        message:
          '카메라가 다른 앱에서 사용 중이거나 장치 접근에 실패했습니다. 다른 앱을 종료하고 다시 시도해주세요.',
      }
    case 'AbortError':
      return {
        trackingState: 'error',
        debugState: 'camera start aborted',
        message: '카메라 시작이 중단되었습니다. 장치 연결 상태를 확인하고 다시 시도해주세요.',
      }
    case 'OverconstrainedError':
      return {
        ...
        message:
          '요청한 카메라 조건을 만족하는 장치를 찾지 못했습니다. 기본 카메라로 다시 시도해주세요.',
      }
    case 'SecurityError':
      return {
        trackingState: 'unsupported',
        debugState: 'camera blocked by browser security policy',
        message: '현재 컨텍스트에서 카메라 접근이 차단되었습니다. HTTPS 또는 localhost 환경에서 열어주세요.',
      }
